...
function * encode(str: Iterable<string>, grid: Map<string, Point>): Generator<string> {
	...
	for (const char of str) {
		...
		// true: join them by an L mirrored horizontally instead
...
function solve(input: string): number {
	...
	for (const line of input.trim().split(/\r?\n/g)) {
		...
		let encoding = encode(line, NUM_PAD);

		for (let i = 0; i < 25; i++) {
			encoding = encode(encoding, ARROW_PAD);
		}

		result += encoding.reduce((acc) => acc + 1, 0) * num;
	}
