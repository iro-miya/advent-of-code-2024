...
interface Connection {
	...
	// The references of every unique point crossed
	// This allows us to know how many are crossed at the end
	points: Set<number>,
	score: number;
			...
			for (const [outward, end] of [[con1, con2], [con2, con1]]) {
				...
				for (const start of startConnections) {
					...
					if (start.end === ref) {
						...
						start.points = start.points.union(end.points);
...
function solve(input: string) {
		...
		grid.forEach((line, index1) => {
			...
			line.forEach((char, index2) => {
				...
				if (char !== "#" && currentConnection != null) {
					...
					currentConnection.points.add(pointToRef(x, y));
				...
				if (
					...
					if (currentConnection.score > 0) {
						...
						const mirror: Connection = {
							...
							points: new Set(currentConnection.points),
				...
				if (char !== "#" && currentConnection == null) {
					currentConnection = {
						...
						points: new Set<number>().add(pointToRef(x, y)),
						score: 0,
						startDirection: direction,
						endDirection: direction,
					}
	...
	const winners: Set<Attempt> = new Set();
	...
	while (attempts.size > 0) {
		...
		// If the best attempt has a higher score than the lowest winning score,
		// then we found every lowest winning paths.
		if (bestAttempt.score > winningScore) {
			break;
		}

		// If the attempt turns out to have reached the end, then we found a winner
		if (bestAttempt.location === end) {
			if (bestAttempt.score < winningScore) {
				// If it improves on the lowest winning score, then we clear the current winners
				winningScore = bestAttempt.score;
				winners.clear();
			}

			winners.add(bestAttempt);
			continue;
		}
		...
		for (const connection of connections) {
			...
			// Since we must find every winning path, we keep the paths with equal score
			if (newScore > highScore) {
				continue;
			}

			attempts.add({
				history: new Set(bestAttempt.history).add(connection),
	...
	// Counts the number of unique points visited
	let visitedPoints: Set<number> = new Set();

	for (const winner of winners) {
		for (const connection of winner.history) {
			visitedPoints = visitedPoints.union(connection.points);
		}
	}
	...
	/*
	for (const [x, y] of visitedPoints.values().map(refToPoint)) {
		level[y][x] = "O";
	}

	return level.map(line => line.join("")).join("\n");
	//*/
}
